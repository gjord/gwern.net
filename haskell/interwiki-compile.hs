import Network.URI (escapeURIString, isAllowedInURI, isUnescapedInURI, unEscapeString)
...
rw :: String -> IO ()
rw f = do 
             let newpndc = changeLinks $ readMarkdown defaultParserState f
             putStr (writeMarkdown defaultWriterOptions{writerWrapText=False} newpndc)

changeLinks :: Pandoc -> Pandoc
changeLinks = bottomUp (map convertInterwikiLinks)
  ...
  case interwiki of
    ('!':interwiki') ->
        case M.lookup interwiki' interwikiMap of
            ...
            where interwikiurl a u = escapeURIString isAllowedInURI $ u ++ a
