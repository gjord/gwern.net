...
main = do  hakyll $ do
           ...
           _ <- runCommand "find _site/ -type d \\( -name _darcs \\) -prune -type f -o -not -name \"*.png\" -not -name \"*.jpg\" -not -name \"*.pdf\" -not -name \"*.avi\" -not -name \"*.svg\" -exec /bin/sh -c \"gzip --stdout --best --no-name --rsyncable \\\"{}\\\" > \\\"{}.gz\\\"\" \\;"
           return ()
